...
def main():
    print(f"[{SERVICE_NAME}] Indul, feliratkozás nfl.raw.audio + nfl.metadata topicokra...")
    wait_for_kafka()
    ...
    def handle(msg):
        ...
        if len(rms_history) == rms_history.maxlen:
            ...
            if rms > MIN_ABS_RMS and ratio > TRIGGER_RATIO and is_new_enough:
                ...
                clip_path = cut_clip(t_sec, confirmed)
                if clip_path is None:
                    rms_history.append(rms)
                    return

                event = {
                    "trigger_time_sec": round(t_sec, 1),
                    "rms": round(rms, 4),
                ...
                print(f"[{SERVICE_NAME}] TRIGGER [{t_sec:.1f}s] ratio={ratio:.2f}x -> {status} "
                      f"-> {clip_path}")
